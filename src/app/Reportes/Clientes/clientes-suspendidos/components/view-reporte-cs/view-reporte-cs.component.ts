...
import { SuspendidosService, ClienteSuspendido } from '../../services/suspendidos.service';
...
@Component({
  selector: 'app-view-reporte-cs',
  templateUrl: './view-reporte-cs.component.html',
  styleUrls: ['./view-reporte-cs.component.css']
})
export class ViewReporteCSComponent {
  ...
  constructor(public suspendidosService: SuspendidosService) {
  ...
  obtenerLocalidades(): void {
    this.suspendidosService.obtenerLocalidades().subscribe(
      (Localidad: Localidad[]) => {
  ...
  generarPdf() {
    if (this.selectedLocalidadId) { // Verifica que haya una localidad seleccionada
      this.suspendidosService.obtenerClientesSuspendidos(this.selectedLocalidadId).subscribe(
        (clientesSuspendidos: ClienteSuspendido[]) => {
          //console.log(clientesSuspendidos); // Verifica los datos aquí
            ...
            columns: [
              {
                ...
                stack: [
                  {
                    text: 'LISTADO CLIENTES SUSPENDIDOS',
                  },
                  {
                    canvas: [
                      createLine(0, 2, 250),
          ...
          if (clientesSuspendidos.length > 0) {
            const tableData = clientesSuspendidos.map(cliente => [
            ...
            content.push({
              text: 'No hay clientes suspendidos',
              alignment: 'center'
            });
          }
