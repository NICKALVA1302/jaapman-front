...
export class ConsumoAguaPipe implements PipeTransform {
  ...
  transform(value: UserxLoc[], buscarUsuario: string, estadoLectura: number, estadoMedidor: number): UserxLoc[] {
    ...
    const nombreApellidoConcat = (usuario: UserxLoc) => (usuario.nombre.toLowerCase() + ' ' + usuario.apellido.toLowerCase());
    const apellidoNombreConcat = (usuario: UserxLoc) => (usuario.apellido.toLowerCase() + ' ' + usuario.nombre.toLowerCase());
