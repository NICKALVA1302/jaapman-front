...
import { Localidad, UsuarioxLocalidad } from '../models/localidades';
...
export class MaterialService {
  ...
  constructor(private http: HttpClient) {
    ...
    this.myApiOpe = 'api/operador';
    this.myApi = 'api/login/';
  }
  ...
  changePassword(body:any){
    const url = `${this.myAppUrl}${this.myApiOpe}/changePassword`;
    return this.http.put(url,body)
  }

    // obtener localidades
    obtenerLocalidades(): Observable<Localidad[]> {
      return this.http.get<Localidad[]>(`${this.myAppUrl}${this.myApi}obtencion/localidadesRegister`);
    }

      //Metodo para obtener usuario por localidad
  datosPorLocalidad(id_localidad: number): Observable<UsuarioxLocalidad[]> {
    return this.http.post<UsuarioxLocalidad[]>(`${this.myAppUrl}${this.myApiUrl}/localxusu`, { id_localidad });
  }
